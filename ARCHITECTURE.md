...
Supabase Postgres (RLS)  ⇄  apps/worker (Railway): ai_jobs queue —
        ...
        |                                   sweep_trashed_drafts() (ADR 0049 — drafts had a soft
        |                                   delete since 20260814000001 and no sweep, so a
        |                                   "deleted" draft was kept forever and shown nowhere) +
...
WRITE PATH (client → HTTP → route handlers; nothing client-side touches the DB):
  ...
  app/api/trash · /restore · /purge · /delete  the whole Trash as ONE list (ADR 0049,
                                               migration 20260821000001). GET fans a
                                               SECURITY INVOKER trash_items() union over
                                               projects + boards + assets + content_drafts,
                                               filtered/sorted/paged/counted in SQL because
                                               "largest first" and an honest total across four
                                               tables cannot be assembled in a browser — the
                                               500-row-no-total read it replaces silently
                                               truncated, and "Empty trash" then purged the
                                               first 500. It returns R2 KEYS; lib/trash.ts
                                               presigns the rendered page alone. The three POSTs
                                               take (kind, id) pairs and fan out server-side,
                                               since restore is per-kind (a photo flips a status
                                               enum, a project clears a timestamp, a photo is
                                               PURGED by enqueuing a worker job). /delete is the
                                               inverse of /restore so a restore can be undone,
                                               and /purge is the first path by which a trashed
                                               PROJECT can be deleted at all. Read seam:
                                               lib/trash.ts; fan-out: lib/trash-actions.ts
  ...
  app/api/topics/recluster                     POST: re-run the workspace's clustering now.
                                               NO UI ENTRY POINT since 2026-09-01 — the
                                               Re-cluster button was removed from
                                               SortingActionBar and the route kept
                                               deliberately (ADR 0038's amendment)
  ...
  app/api/imports                              picked cloud files → assets+files → ONE
                                               ...
                                               original once INTO R2. onedrive (ADR 0047):
                                               shaped like gdrive — own connection, r2_key
                                               null — except an item may be a FOLDER, which
                                               becomes `payload.onedrive_expand` rather than
                                               an asset, because a folder can hold 18,000
                                               files and this route caps at 500
  app/api/sources/onedrive                     GET status · DELETE disconnect (ADR 0047). The
                                               DELETE only forgets OUR side: Microsoft exposes
                                               no programmatic revocation, so the response says
                                               revokedUpstream:false and the copy tells the user
                                               where to finish the job
  app/api/sources/onedrive/oauth · /callback   the connect flow — a full-page REDIRECT, not
                                               Drive's popup. Forced by the app registration:
                                               only a confidential `Web` client yields a refresh
                                               token that outlives a day (a `spa` redirect URI
                                               caps them at 24 h, stranding the worker). PKCE +
                                               a session-bound state in httpOnly cookies; the
                                               callback reports a first-party CODE only, never
                                               Microsoft's error_description (ADR 0021)
  app/api/sources/onedrive/browse              one folder's children, server-side (ADR 0047 D1).
                                               The Graph token never reaches the browser. This
                                               is our own list instead of Microsoft's v8 picker:
                                               the picker is not an SDK but a hand-rolled
                                               postMessage protocol needing a SharePoint-resource
                                               token that is NOT the Graph token, and listChildren
                                               had to exist anyway for the folder walk
...
- **Group** — the Topic view's cloud key. For real assets it is the **stored semantic cluster label** when the asset has one: the `cluster` worker job runs deterministic k-means over the image embeddings and writes `topic_clusters` + `assets.cluster_id`, labelling each cluster by its most *representative and* discriminative tags — score = support × lift, medium/format words demoted, second tag only if it earns it (ADR 0028 as amended by **ADR 0038**, which is why a cluster of screenshots stopped being called "book cover · price tag"). These clusters are **stable across sessions and identical in every project** of a workspace — "yoga"/"stretching"/"йога" merge into one cloud — and `lib/assets.ts` reads the label through an embedded `topic_clusters ( label )` join (RLS nulls a cross-workspace cluster, so the join only ever surfaces the caller's own labels). When an asset has no cluster yet (analyzed but not clustered, or added after the last run), `deriveTopics` (`lib/topics.ts`) falls back to the **tag heuristic** (ADR 0023): the most-shared viable tag in event → scene → object priority, ambient tags skipped (but an asset whose only thematic tags are ambient keeps one rather than falling to `Other`), unanalyzed assets → `Unsorted`. The top-6 + Other cap now bounds **only** those heuristic topics: a stored cluster label always keeps its own cloud (ADR 0038), because the heuristic is result-set-relative (counts run over the current project's newest ≤500 rows) while the clusters are not — they are computed once over the whole workspace, so folding one meant a photo's stable semantic home depended on which project you opened. A cloud can also be **renamed** by the user (double-click its label → `PATCH /api/topics/[id]`; `topic_clusters.is_renamed` pins it against every future re-cluster) and **re-clustered on demand** (`POST /api/topics/recluster`, zero credits) — though that route has had **no UI entry point** since 2026-09-01, when the Re-cluster button was removed from `SortingActionBar` (ADR 0038's amendment); clustering still runs automatically after every analyze. The old fixed keys (rescue, aid, urban…) survive only as mock seeds with curated `GROUPS` colors.
- **OneDrive import** — the third connection-backed source (ADR 0047, migrations `20260817000001` + `20260817000002`), and the only one that can import a **folder tree**. That is the whole reason it exists beside Drive: Google's `drive.file` cannot expand a picked folder into its children, so Drive import makes a user multi-select thousands of files by hand, while delegated `Files.Read` — the *least-privileged* Graph permission for `/children` on both personal and work accounts — walks the tree. Structurally it is Drive's twin (own connection, encrypted refresh token, `r2_key` stays null, worker re-reads bytes at processing time) with four differences. **(1) No v8 picker.** We browse Graph ourselves (`/api/sources/onedrive/browse` → `components/import/OneDriveBrowser.tsx`), because the picker is not a drop-in SDK like Drive's and Dropbox's but a hand-rolled `postMessage` protocol against a *SharePoint-resource* token that is deliberately not interchangeable with the Graph token, on a host that differs per account type. `listChildren` had to exist anyway for the walk, so browsing is that call with a breadcrumb — and the token never leaves the server. The cost is real and accepted: no "Recent", no "Shared with me". **(2) Connecting is a full-page redirect**, not a popup: only a confidential `Web` app registration yields a refresh token that outlives a day, since a `spa` redirect URI caps them at 24 h and would strand the worker mid-import. **(3) A picked folder is not an asset** — it rides as `ingest.payload.onedrive_expand`, and the worker walks it (`handlers/onedrive-expand.ts`), fanning the discovered files out into further `ingest` jobs of ≤500. No new job type (the seven-handler invariant holds), retry granularity and ADR 0032's containment survive, and because those child jobs carry `asset_ids` only, an expansion can never enqueue another expansion. Depth and item caps **fail loudly naming the limit** rather than truncating — a silent cut reads as "OneDrive only had 5000 photos". **(4) Graph's `photo`/`location` facets are a fallback, never truth**: local `exifr`/`exiftool` extraction runs on the real bytes and wins every field it answers, with facets filling only its gaps (`mergeExifFallback`), because OneDrive for Business and SharePoint return *only* `takenDateTime` and facet coverage on RAW/HEIC is unverified. Throttling is a **connection-scoped circuit breaker** (`ThrottleGate`), not per-request backoff, because Graph's own guidance is to pause *all* requests on a 429 — and 503 counts as throttling too. Both account types run one code path; the business half is untested for want of a work account. Zero credits: a fetch is not an AI action.
...
- **Workspace** (a `board` in code, so it cannot be confused with the tenant `workspace_id`) — a named, colour-coded set of a project's files (`lib/boards.ts`, `hooks/useBoards.ts`, ADR 0044). A `boards` row since migration `20260812000001`; the header's `BoardBrowser` is a chip per workspace, and **the project name to its left is the "all files" scope** — clicking it leaves the open workspace, while a caret beside it opens the project switcher (the `All files` chip that used to say the same thing is gone; on the workspace-wide `all` canvas the control stays a plain switcher). A new workspace takes the lowest free `Workspace N` and the first unused colour, both reserved by creates still in flight, so a double-click on `＋` cannot mint two identical ones. A chip's **dot opens the seven-colour `LabelSwatchRow`** (right-click on the chip does too) — the same swatch object a photo's label and a note's paper use, `clearable={false}` because a workspace always has a colour; the popover is `position: fixed` since the breadcrumb clips its own overflow. The breadcrumb takes whatever the header's right-hand tools leave rather than stopping at a fixed cap, so names only ellipsize when the room genuinely runs out. Opening one **narrows the canvas to its files and does nothing else** (bar one signal: the background grid switches from dots to ruled lines, so a narrowed canvas never looks like a full one) — notes, folders, artboards, all four views, export and the label filter behave exactly as with none open. The scope is applied at `canvasPhotos()`, the set every layout reads, and mirrored into `WorkspaceState.boardScope`. That is the **opposite** of the label filter and the difference is the point: a filter hides tiles and leaves every position intact (`visibleTilePositions`, the render seam), whereas a workspace has to RE-PACK, or six photos sit at the coordinates they had among four hundred. Re-packing means the geometry seam (`activeTilePositions` — drags, folder drops, artboard membership, export order) and the render seam must run over the same set, or a drag lands where nothing is drawn. An open workspace with no files gets its own empty state, gated ahead of the project-empty one. **Each scope keeps its own coordinates** (ADR 0044 amended 2026-08-17): the persisted arrangement is one `localStorage` blob per project *plus* open workspace (`lib/canvas-store.ts` — `archivemind:canvas:{projectId}` stays the project's own pre-workspace key, `…:b:{boardId}` is a workspace), holding the five `galleryOverrides` buckets, the legacy `frames`, the folder boxes (`groupGeom`) and `tileZ`. Sharing one bucket meant arranging ten photos inside a workspace re-arranged the same tiles on the project canvas they were carved from — the ADR shipped that as a stated consequence and this closes it. Two rules make it correct rather than merely keyed: `switchCanvasScope` **flushes the outgoing scope under its own key before loading the incoming one** (writes are debounced 400 ms, so a drag then a chip click inside that window is exactly the case that would lose work — the switch *is* the save), and the key rides in `WorkspaceState.canvasScope` **beside** the arrangement it describes, so the debounced save can never write the set you just left under the key of the one you just opened. A never-arranged scope opens as a clean grid, and undo history is dropped on a switch — a snapshot's coordinates mean nothing over another set. A **note** is the exception that stays shared: its geometry is server-held content (ADR 0041), one x/y read by both the project canvas and its owning workspace. A **folder box** is not — the server owns membership, the browser owns the box, so it moves with the scope. **Switching workspace re-frames the canvas** — the same fit the `Fit` button performs, run from an effect on the committed `boardScope` (never on the `activeBoardId` prop, which changes a render earlier and would frame the set you just left). Re-packing is exactly why: without it the camera still points at coordinates that belonged to the previous set, so six photos out of four hundred land off-screen and the canvas reads as empty. A membership change does not re-fit (the ref keeps it to real switches — yanking the camera as a file is dropped on a chip would be its own bug), and neither does opening an empty workspace, which has nothing to frame. **Deleting one is a three-part affair** (ADR 0044 amended 2026-08-13, migration `20260813000001`), because the `×` that does it sits on the chip you click to *open* a workspace: it asks first (`ConfirmModal`, naming what survives), it stamps `boards.deleted_at` instead of removing the row (`PATCH { deleted: true }` — the project trash's own idiom; membership stays and the notes and folders keep pointing at it, which is what makes a restore whole rather than a name over an empty canvas), and the undo rides on that delete's **own toast** (`Undo` beside "*Name* moved to Trash"), the shape every reversible delete already uses — deliberately not a second `↺` in the header, which read as a duplicate of the canvas undo a few hundred pixels away rather than as a different job. `DELETE /api/boards/[id]` is now the permanent path and is reachable only from the Trash panel, where trashed workspaces list above the trashed photos; that panel and not the homepage Trash view, because a workspace belongs to one project while a trashed photo is workspace-global. `sweep_trashed_boards()` hard-deletes past the 30-day window on the worker's usual sweep, and only then do the FK side-effects (membership cascade, `board_id` nulled) run.
- **Create hub** — the Workspace's one outcome surface (`CreateHubDialog`, ADR 0045 as amended twice). One floated button, `EXPORT`, badged with the draft count; opening a Workspace is the only way to see it. The button read `CREATE` until the project canvas took the same top-right slot with `ANALYZE`: between the two scopes the axis a reader needs is *bringing files in and understanding them* (outside a Workspace) versus *taking work out* (inside one), so the word on the door names the scope's job. Only the label moved — the hub still opens on **Make something**, which is the one place its name and its first section disagree. It answers a single question — *what comes out of this Workspace* — in three blocks, and the axis between the first two is **editable versus final**, not create-versus-download: *Make something* (Article, Instagram carousel) goes hub → brief → studio and stays rewritable; *Download* (PDF document, Captions CSV, The files) opens `ExportDialog` on that format and hands over a finished file; *Continue editing* lists the saved drafts. The delivery cards are the export dialog's own **Format row promoted one level**, which is why folding DOWNLOAD in here cost only the default PDF a click — CSV and ZIP always took two — and why the row stays inside the dialog: a card is a preset, not a branch, so changing your mind needs no step back. Scope is stated once, on the Download heading (`5 selected` / `48 files`), under the rule the old button used: the selection when there is one, the whole Workspace otherwise. `outputUi` in `ArchiveWorkspace` is the state machine — `hub | brief | download | studio` — and its `download` rung is what makes the dialog's dismiss read `‹ Back` and Escape return to the hub; the SAME dialog opened from the right-click menu, the drawer or the studio leaves `outputUi` alone and closes to where it came from. Those two are the fast paths that survive the merge: `Download N` on a selection, and one photo from the drawer.
- **Trash** — one typed list of everything soft-deleted, not a place per kind (ADR 0049, migration `20260821000001`). Four tables carry a 30-day soft delete — `projects`, `assets`, `boards`, `content_drafts` — and until this they were surfaced by three different pieces of UI and, for drafts, by none at all. A `TrashItem` is a discriminated union over `project | workspace | asset | draft` carrying name, thumb, **size**, **location** (where Restore puts it back), **deleted-by** and expiry; `components/trash/` renders it as a card (grid) or a row (list), and the in-canvas `TrashPanel` renders the same rows as a narrower slice — so the countdown, the type label and the "restores to" line are computed in exactly one place for both. The chip key is the item's kind **except for an asset, where it is the asset's own `asset_kind`**: the filters read `Photos · PDFs · Documents · Other files · Projects · Workspaces · Drafts` without `asset` ever being a chip, and a future member of the enum becomes a chip with no code change. Filtering, sorting, paging and the per-chip counts all happen in `trash_items()` (SECURITY INVOKER, RLS is the boundary) because sorting by size across four tables and an honest `total` cannot be assembled in the browser — counts deliberately ignore the *type* filter, or every chip but the active one would read zero. **A destructive button may only act on what the filter matches, and must name the number**: "Empty trash" becomes "Delete all (N)" and collects the ids across pages before the confirmation quotes N. That is the opposite of the label filter's rule (ADR 0040) on purpose — a canvas filter is a way of looking at an arrangement, a Trash filter is how you pick what to destroy. `deleted_by` is stamped by a **trigger**, never a route (ADR 0033's reasoning for `deleted_at`: `assets` has four status writers), and cleared on restore. `sweep_trashed_drafts()` exists because listing drafts without one would print a countdown that never runs out; publication links are untouched, carrying their own snapshot (ADR 0046). Workspaces stay on the header's own board state in the panel — restoring one has to put its chip back on the same frame — and stay out of the homepage view for ADR 0044's reason: a Workspace belongs to one project.
- **Label** — a **colour label**: one of seven macOS colours a *person* puts on a photo. Read on **every** surface a photo appears on: the canvas tile draws it beside the filename, and a Map marker draws it on the photograph itself, top-left inside each mosaic cell (ADR 0040's 2026-09-01 amendment — Map could filter by a colour without ever drawing one, which is exactly the gap that made "a marker you read in every view" untrue) (`assets.label`, ADR 0040, migration `20260808000001`). Deliberately not a tag: `tags` is the analyze handler's output and feeds the Topic heuristic (ADR 0023), the connecting-line web (ADR 0022) and search's explicit tier (ADR 0029/0031), so a row named `red` in there would grow a "red" cloud, wire every marked photo together, and answer a text search for the colour. Single-valued, because a tile has to be able to answer which colour it carries with one value — the retired LABELS view needed it to pack a cloud, and the filter still needs it to decide what to hide. Assigned from one swatch row that appears in four places — the top of the right-click menu, both bottom action bars, the drawer, and the keys `1`–`7` (`0` clears; bare digits, since `⌘1`–`⌘7` switches browser tabs) — all funnelling through `POST /api/assets/label` with an Undo toast that restores each photo's *previous* colour. The seven names are still renameable per workspace in the data (`workspace_labels`, `PATCH /api/labels`; only renamed colours have a row, the defaults live in `packages/shared`) but have **no UI entry point** since the filter panel and the LABELS view were retired — deliberate, recorded in ADR 0040's amendments, not a control that went missing. Costs no credits (ADR 0037: a credit is one AI action on one photo; no model runs here).
...
- **AI actions** — every AI entry point (tile ✨ badge, the Workspace action bar's ✨, the project canvas's floated `ANALYZE`, the drawer) plans its run through the single pure `lib/ai-ops.ts` `planAiRun`, which returns both the jobs to enqueue *and* the button text, so a label can't describe work the run won't do. `ops.tags` → `analyze`, `ops.captions` → `caption`, both → analyze **chained** into caption (the caption prompt reads the facts analyze writes, so they can't be one job; `useWorkspace`'s `followUpCaption` ref fires the second leg when the first reports done). Two operations exist because two job types exist — the panel's old "Detect & group faces" checkbox had neither a job type nor a handler and is gone. Tile badges read `photo.processed` (= `ai_processed_at`, written by analyze only) and clicking one analyzes that photo; `aiBusyIds` marks the tiles inside the running job. The two BULK entry points are scope-exclusive and between them cover the whole canvas: the action bar's ✨ inside a Workspace, the floated `ANALYZE` outside one (`components/bulk-ai/CanvasAnalyzeAction.tsx`, badged with the selection count). Both open `BulkAiPanel` rather than running anything, because analysis costs a credit per photo and the count, the operations and the call estimate have to be visible before the spend. Neither the left toolbar nor the right-click menu is one of these: the toolbar's ✨ opens the AI **assistant** (Smart Search), and the menu's analyze item went with the trim in #233 — which is what left the project canvas with no bulk AI at all until `ANALYZE` arrived.
...
the queue without one — plus six retention sweeps on the single 6-hourly tick,
caught independently so a broken one cannot stall its neighbours: trashed
projects, trashed Workspaces, trashed content drafts (ADR 0049), trashed assets,
export artifacts past `EXPORT_RETENTION_DAYS`, and publication share previews; `packages/shared` holds live zod contracts both
