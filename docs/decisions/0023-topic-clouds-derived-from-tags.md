...
- **Ambient tags are skipped as candidates**: a tag on more than
  `TOPIC_AMBIENT_FRACTION` (60%) of the tagged assets names the whole archive,
  not a theme inside it (floor of 2 so tiny archives still get a topic).
- Tagged assets with no viable thematic tag → **`Other`**.
- Only the **`TOPIC_CLOUD_CAP` (6) biggest topics keep their name** (size desc,
  name asc), the rest fold into `Other` — matches the 6-color cloud palette and
...
- **Topics are workspace-relative and re-derived on every read** — analyzing
  new photos can rename or re-split clouds (an asset's "most shared tag"
  changes as the corpus grows). Acceptable for an exploration canvas; the
  stable version of this field is the embedding-clustering job (spec §13),
...
- Topic drag overrides (`galleryOverrides.topic`) key on asset ids, not cloud
  ...
  where they were dropped — same behavior as any re-layout.
- The `GROUPS` lookup table now only serves mock seeds and the (unreachable)
  source-browser sidebar — one step closer to retiring the
  `lib/mock-data.ts` debt imports.
