...
import { buildCaptionPrompt, type CaptionContext } from "./caption";

const empty: CaptionContext = {
  taken_at: null,
  camera_make: null,
  camera_model: null,
  gps_label: null,
...
describe("buildCaptionPrompt", () => {
  it("carries the style template and target language", () => {
    const p = buildCaptionPrompt("agency", "uk", empty);
  ...
  it("includes only the metadata that exists", () => {
    const p = buildCaptionPrompt("social", "en", {
      ...empty,
      taken_at: "2026-06-18T14:03:00Z",
      camera_make: "Nikon",
      gps_label: "Kyiv, Ukraine",
    });
  ...
  it("accepts a Date for taken_at (pg parses timestamptz into Date)", () => {
    const p = buildCaptionPrompt("archival", "en", {
      ...empty,
      taken_at: new Date("2026-06-18T14:03:00Z"),
    });
    expect(p).toContain("Taken: 2026-06-18");
  });
  ...
  it("joins confirmed facts and anchors the no-invention rule", () => {
    const p = buildCaptionPrompt("archival", "en", {
      ...empty,
      confirmed_facts: ["rescue operation", "flooded street"],
    });
