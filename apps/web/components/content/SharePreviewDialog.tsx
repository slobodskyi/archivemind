...
function SharePreviewDialogContent({
  ...
  const footer = result && ended ? (
    <>
      <DialogButton onClick={onClose} disabled={busy}>Close</DialogButton>
      <DialogButton data-autofocus="" variant="primary" onClick={onDismissResult} disabled={busy}>
        Create a new link
      </DialogButton>
    </>
  ) : result ? null : (
    <>
      ...
      <DialogButton data-autofocus="" variant="primary" onClick={() => onCreate({ expiresInDays, allowDownloads })} disabled={busy}>
  ...
  return (
    <Dialog
      open
      size="m"
      kicker={draftKind === "article" ? "Article preview" : "Carousel preview"}
      title={`Share “${draftName}”`}
      subtitle="Create a view-only preview link anyone can open — no ArchiveMind account needed."
      busy={busy}
      onClose={onClose}
      footer={footer}
    ...
    >
      {unmanagedShares.length ? (
        <div style={{ marginBottom: 16, padding: "12px 13px", border: "1px solid rgba(255,184,77,.35)", background: "rgba(255,184,77,.06)", borderRadius: 2 }}>
          ...
          {unmanagedShares.map((share) => (
            <div key={share.shareId} style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 10, paddingTop: 10, borderTop: "1px solid var(--bd)" }}>
              ...
              <button
                ...
                disabled={busy}
                style={{ ...buttonStyle, flex: "0 0 auto", borderColor: "transparent", color: "var(--red)", fontSize: 10.5, opacity: busy ? 0.5 : 1 }}
              >
                Turn off
              </button>
            </div>
      ...
      {result && ended ? (
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 6, marginTop: 16 }}>
            ...
            />
            <button type="button" onClick={copyLink} disabled={busy} style={{ ...buttonStyle, minWidth: 86, background: "var(--ac)", border: 0, color: "#050505", fontWeight: 800, opacity: busy ? 0.55 : 1 }}>
